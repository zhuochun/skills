...
6. **Build a feasible portfolio.** Reserve or expose capacity for obligations and enablement, then compare opportunity bets against the remainder. Check skills, dependencies, integration, technical risk, operations, and iteration. Neither ease nor difficulty establishes value. Treat effort as a cost or constraint; justify necessary complexity by the outcome it enables, including costs shifted to users, operators, or maintainers.
