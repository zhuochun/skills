...
10. **Route the accepted direction.** Send weakly framed or weakly evidenced opportunities to `product-opportunity-discovery`. Send accepted but behaviorally ambiguous changes to `software-change-specification`, multi-team outcomes to `technical-program-execution`, consequential transitions to `high-risk-change-planning`, changed shared interfaces to `software-contract-evolution`, and clear bounded implementation to `scoped-change-implementation`. Prioritization does not take over those specification or execution contracts.
