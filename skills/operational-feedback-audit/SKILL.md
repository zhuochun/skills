...
Evaluate whether telemetry enables correct, timely decisions. Data collection and dashboard count are not operational feedback.

## Preserve authority and data safety

- Audit by default; do not change alerts, retention, sampling, routing, or production configuration without authorization.
...
1. **Define the service promise.** State expected useful behavior, its owner, and important populations or workflows.
...
3. **Evaluate health orientation.** Check whether an unfamiliar responder can start from a small, comparable set of customer-proximate signals before specialized diagnostics.
...
5. **Reconstruct runtime structure.** Compare declared dependencies with observed graphs, context, queues, retries, fanout, deployments, and ownership.
...
7. **Evaluate alert routing.** Check impact, affected slice, owner, recovery state, recent changes, dependency evidence, uncertainty, and drill-down. Alerts need not prove root cause.
...
- Multi-component or owner loops include the smallest self-contained text comparison of declared and observed paths through promise, signal, diagnosis, routing, control, and learning. Keep missing links explicit.
...
- Retire misleading signals; report contract defects and route design changes to `observability-design`.
