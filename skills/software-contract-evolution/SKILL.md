...
8. **Define transition obligations.** State the old, mixed, and target contract states, compatibility invariants, authority rules, and conditions needed for cutover. Route detailed phases, write authority, rollback, compensation, and cleanup sequencing to `high-risk-change-planning`; route multi-team adoption flow and dependency steering to `technical-program-execution`.
...
- `high-risk-change-planning` owns the executable old-to-mixed-to-new transition, cutover, recovery, and cleanup sequence. This skill supplies the compatibility matrix, support policy, consumer obligations, and retirement claims.
- `technical-program-execution` owns multi-team slices, dependencies, integration, decision latency, and replanning for consumer adoption. It does not redefine the contract.
