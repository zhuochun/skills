...
- Act as the sole primary executor for a slice that intentionally changes supported observable behavior. Route a pure structural ownership change to `behavior-preserving-refactoring` instead of wrapping that executor in this workflow. For a mixed slice, follow the orchestrator's nominated primary executor and borrow only the necessary preservation or behavior checks from the neighboring contract.
- When orchestration supplies accepted scope, specification claims, design, assurance posture, and valid evidence, consume them and produce the implementation delta. Reopen framing only when current evidence contradicts it or a missing decision blocks the authorized behavior change.
...
2. **Consume or define the change contract.** Use existing `CHG-*` claims or another accepted specification or owner decision when supplied; do not reopen settled questions unless current evidence shows that the prior decision does not cover the discovered case or rests on a material contradiction. Otherwise name the behavior to add or alter, invariants to preserve, affected consumers, compatibility expectations, required controls, completion evidence, and explicit non-goals. Reconcile differences among current behavior, tests, documentation, persisted data, and consumer expectations before treating them as blockers. Stop only when the remaining conflict would require choosing a consequential behavior, compatibility, state, security, data, or recovery policy beyond existing authority. Name the competing observable outcomes and smallest decision needed, then route to `software-change-specification` or the narrower owning design skill. Reference existing `CHG-*`, `VER-*`, `OBS-*`, or release contracts by key and plain-language label when present.
...
7. **Verify after every slice.** Reuse valid snapshot-matched evidence and run the narrowest meaningful check needed for the new delta first. Inspect the result, then continue from green evidence or repair the latest step. Expand to affected integration, contract, static, build, and repository checks only as the change surface grows. Record what was actually executed; do not convert a planned or skipped check into evidence or rerun a still-valid check merely because this workflow began.
