...
- When one directly discoverable owner, file, symbol, command, simple call path, or focused test can answer the request, perform only that lookup. Stop when the requested owner, path, or verification seam is established; do not produce a map or load the reference.
...
- Label evidence as observed, executable, declared, reported, or inferred. Record recency and disagreements. Trace ownership and live-state claims to the actual authority or state path; do not infer them from file placement, names, or topology.
...
10. **Close scenario paths.** Walk the normal path and any change, degraded, failure, or recovery scenario that can alter the named decision. Continue each scenario to the next usable state or an explicit unknown. Include derived state and protective controls only when they can change the outcome, continuity, or recovery.
