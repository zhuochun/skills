...
2. **Recover evidence for the question.** Start from supplied evidence, constraints, prior decisions, and the current customer uncertainty. Use a provisional opportunity or competing hypotheses to select further observation, interviews, support, analytics, experiments, market evidence, or abandoned attempts. Seek counterevidence; stop collecting when another source cannot change the disposition, confidence, or next learning move. Distinguish observation, preference, behavioral measure, inference, and unknown; preserve contradictions, sampling limits, and freshness.
