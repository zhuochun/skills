...
8. **Preserve the tested mechanism.** Define the smallest learning artifact that still exercises the value-producing behavior, decision, or interaction. When authorized local software construction is the evidence medium, route it to `prototype-to-learn`. State what the artifact omits and why those omissions do not invalidate the targeted learning. A polished but behaviorally empty prototype is not cheap evidence.
