...
8. **Route specialized judgments.** Use `decision-facilitation` for contested consequential clarification, challenge, and closure. Let `migration-planning` own accepted technical movement through controlled transition states, authority transfer, retireability, and retirement; let `controlled-release-design` own exposure control, `observability-design` own measurement contracts, and `verification-strategy-design` own claims and oracles. Reference stable keys with plain-language labels rather than copying plans.
