...
7. **Compare equivalent dimensions.** Normalize claim, method, population or workload, setting, intervention or mechanism, outcome, window, assumptions, limitations, and transfer boundary. Explain disagreement from conflicting results, different questions, incomparable contexts, versions, or incentives. Compare provenance, evidence overlap, source incentives, and information uniqueness. Correlated repetition is not independent support. Preserve a pivotal minority claim for focused verification when the majority lacked access to its evidence; dissent is neither proof nor a reason for automatic dismissal. Source counts do not settle weight.
...
- `technical-growth-coaching` owns engineering mental-model explanation, misconception repair, reconstruction, and transfer checks. It may consume supplied or synthesized evidence; teaching does not establish source truth.
