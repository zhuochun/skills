...
| [`software-security-design`](../skills/software-security-design/) | Risk-proportionate secure-by-design and threat-modeling decisions, security requirements, secure defaults, control ownership, and assurance obligations before implementation | Evidence-ranked security frame, threat paths, `SEC-*` requirements, controls and exceptions, verification obligations, residual risks, and disposition |
...
| Skill | It owns | Primary output |
| --- | --- | --- |
| [`goal-pursuit`](../skills/goal-pursuit/) | Adaptive end-to-end pursuit of one explicit non-trivial goal through proportional system planning, minimum sufficient capability selection, and goal-level evidence | Parent goal contract, compact pursuit model, immediate checkpoint, linked specialist results, outcome-level evidence, and disposition |
...
| [`software-change-orchestration`](../skills/software-change-orchestration/) | Resumable routing and continuity across sessions, artifacts, or specialist owners for one bounded software change | Outcome and authorization boundary, derived assurance posture and workflow budget, optional canonical specification, exclusive executor route, reusable evidence, and durable handoff state when needed |
| [`technical-program-orchestration`](../skills/technical-program-orchestration/) | Multi-workstream outcome coherence, owned local loops, dependency and integration flow, evidence-labeled constraints, and replanning | Canonical program surface, delivery topology, workstream contracts and frontiers, dominant or coupled constraint model, integration state, evidence trend, and closure |
...
| Skill | It owns | Primary output |
| --- | --- | --- |
| [`agentic-repo-setup`](../skills/agentic-repo-setup/) | Bootstrap around accepted product and technical constraints, retrofit, and renewal of the repo-local harness that makes coding-agent work discoverable, executable, and verifiable | Effective `AGENTS.md` routing, claim-level authority placement and retrieval, stable commands, explicit focused and full evidence states, gate scope deltas, compatibility-safe path disposition, and remaining external actions |
