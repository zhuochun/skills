...
| Skill | It owns | Primary output |
| --- | --- | --- |
| [`agentic-repo-setup`](../skills/agentic-repo-setup/) | Bootstrap, task-centered retrofit, and failure-driven renewal of the repository foundation for coding-agent work | Effective `AGENTS.md` routing, minimum sufficient task-to-authority retrieval, functionally separated repository knowledge, stable commands, risk-matched verification, compatibility-safe path disposition, and a usable bootstrap or a demonstrated improvement or evidence-backed no-op for retrofit or renewal |
