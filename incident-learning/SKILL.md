...
9. **Create branching follow-through.** Classify each learned gap before selecting an intervention. Route detection, diagnosis, or paging failures to operational feedback; signal-contract failures to observability design; escaped behavior to verification; domain or structural gaps to focused design; overload to capacity engineering; rollout failures to release or change planning; ownership and authority gaps to ownership or decision work; and accumulated foundational mismatch to `architecture-consolidation-review`. Run independent branches in parallel when they own different gaps. Update a runbook, automation, or local process directly when that is the smallest sufficient intervention. Assign owners and review dates.
