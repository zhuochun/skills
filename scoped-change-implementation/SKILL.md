...
2. **Define the change contract.** Name the behavior to add or alter, invariants to preserve, affected consumers, compatibility expectations, required controls, completion evidence, and explicit non-goals. Reference existing `VER-*`, `OBS-*`, or release contracts by key and plain-language label when present.
