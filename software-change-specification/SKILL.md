...
7. **Bound coherent delivery.** Identify the smallest useful end-to-end behavior slice and any independently valuable follow-on slices. Name prerequisites and cross-slice invariants, but leave team sequencing and live dependency steering to `technical-program-execution`, and leave old-to-mixed-to-new transition phases to `high-risk-change-planning`.
...
- `controlled-release-design`, `high-risk-change-planning`, `observability-design`, and `verification-strategy-design` own release controls, transition phases, measurement contracts, and falsifying evidence methods respectively.
- `technical-program-execution` owns multi-team execution flow. `scoped-change-implementation` consumes the change specification and owns code changes, feedback loops, deviations, and cleanup.
